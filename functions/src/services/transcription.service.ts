...
export class TranscriptionService {
  private supportedFormats = ['.mp3', '.wav', '.m4a', '.ogg', '.webm'];
   ...
   */
  validateAudioUrl(audioUrl: string): { isValid: boolean; error?: string } {
    try {
   ...
   */
  private isKnownAudioService(url: string): boolean {
    const knownServices = [
      'drive.google.com',
      'docs.google.com',
      'github.com',
   ...
   */
  async transcribeLocalFile(filePath: string): Promise<TranscriptionResult> {
    try {
      ...
      const encodingConfigs = [
        { encoding: 'MP3' as const, sampleRate: 16000, description: 'MP3 16kHz' },
        { encoding: 'MP3' as const, sampleRate: 44100, description: 'MP3 44.1kHz' },
        { encoding: 'LINEAR16' as const, sampleRate: 16000, description: 'LINEAR16 16kHz' },
        { encoding: 'FLAC' as const, sampleRate: 16000, description: 'FLAC 16kHz' }
      ];
      ...
      throw new Error(`Todas as configurações falharam. Último erro: ${lastError?.message}`);
  ...
  async transcribeFromUrl(audioUrl: string): Promise<TranscriptionResult> {
    try {
      ...
      const audioBuffer = await new Promise<Buffer>((resolve, reject) => {
        const makeRequest = (url: string, maxRedirects = 5) => {
          if (maxRedirects <= 0) {
   ...
   */
  private fallbackToMockTranscription(audioUrl: string): TranscriptionResult {
